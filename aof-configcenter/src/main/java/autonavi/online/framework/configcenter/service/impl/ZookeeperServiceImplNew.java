...
public class ZookeeperServiceImplNew implements ZookeeperService {
	...
	private void initAofConfigCenter(String appRoot, ZooKeeper zk) {
		try {
			...
			if (ZooKeeperUtils.checkZKNodeIsExist(zk, SysProps.AOF_ROOT + "/"
				...
				// biz-split
				ZooKeeperUtils.createSafeZKNode(root
						+ SysProps.AOF_APP_BIZ_SPLIT, null);
				...
				ZooKeeperUtils.createSafeZKNode(SysProps.AOF_TEMP_ROOT + "/"
						+ appRoot + "/" + appRoot + "_"
						+ SysProps.AOF_APP_BIZ_SPLIT_STR, null);

				// monitor
				ZooKeeperUtils.createZKNode(SysProps.AOF_MONITOR_ROOT + "/"
						+ appRoot, null);
				ZooKeeperUtils.createZKNode(SysProps.AOF_MONITOR_ROOT + "/"
						+ appRoot + SysProps.TIMESTAMPS, null);
				ZooKeeperUtils.createZKNode(SysProps.AOF_MONITOR_ROOT + "/"
						+ appRoot + SysProps.SERVERS, null);
				ZooKeeperUtils.createZKNode(SysProps.AOF_MONITOR_ROOT + "/"
						+ appRoot + SysProps.VERSION,
						"1".getBytes(SysProps.CHARSET));
				ZooKeeperUtils.createZKNode(SysProps.AOF_MONITOR_ROOT + "/"
						+ appRoot + SysProps.BIZ_VERSION,
						"1".getBytes(SysProps.CHARSET));
	...
	@Override
	public void loginAppRoot(String appRoot, String passwd, String sessionId,
			Boolean isDev) {
		// 从ZK获取应用的校验信息
		try {
			if (!ZooKeeperUtils.checkZKNodeIsExist(zookeeperInit.getZoo(),
			...
			if (pass.equals(passwd)) {
				...
				if (isDev) {
					...
					ZooKeeper zk = zookeeperInit
							.generateAppZoo(appRoot, passwd);
					...
					// 检测是否存在单点登录校验点
					boolean flag = ZooKeeperUtils.checkZKNodeIsExist(
							zookeeperInit.getZoo(), SysProps.AOF_ROOT
									+ SysProps.AOF_PASS + "/" + appRoot
									+ SysProps.LOGIN_FLAG, false);

					// 用户登录加锁
					ZooKeeperUtils.startTransaction(zk);
					ZooKeeperUtils.setACL(generateACL(appRoot, passwd));
					if (!flag)
						ZooKeeperUtils.createSafeZKNode(SysProps.AOF_ROOT
								+ SysProps.AOF_PASS + "/" + appRoot
								+ SysProps.LOGIN_FLAG,
								sessionId.getBytes(SysProps.CHARSET));
					else
						ZooKeeperUtils.setZkNode(SysProps.AOF_ROOT
								+ SysProps.AOF_PASS + "/" + appRoot
								+ SysProps.LOGIN_FLAG,
								sessionId.getBytes(SysProps.CHARSET));
					ZooKeeperUtils.commit();
	...
	private void copyAppNode(String sourcePath, String desPath, ZooKeeper zk,
			List<String> exPath) throws Exception {
		List<String> l = new ArrayList<String>();
		if (exPath == null)
			exPath = new ArrayList<String>();
		try {
			l = zk.getChildren(sourcePath, false);
			for (String url : l) {
				boolean flag = false;
	...
	private void saveBizConfig(Map<String, String> ccBase, String root)
			throws Exception {
		for (String _key : ccBase.keySet()) {
			ZooKeeperUtils.createSafeZKNode(root + "/" + _key, ccBase.get(_key)
					.getBytes(SysProps.CHARSET));
		}
	}

	private void saveBizConfigNew(Map<String, CcBizEntity> ccBase, String root)
			throws Exception {
		for (String _key : ccBase.keySet()) {
			ZooKeeperUtils.createSafeZKNode(root + "/" + _key, ccBase.get(_key)
					.getValue().getBytes(SysProps.CHARSET));
			String comments = ccBase.get(_key).getComments();
			if (comments != null && !comments.trim().equals("")) {
				ZooKeeperUtils.createSafeZKNode(root + "/" + _key
						+ SysProps.AOF_APP_BIZ_COMMENTS,
						comments.getBytes(SysProps.CHARSET));
			} else {
				ZooKeeperUtils.createSafeZKNode(root + "/" + _key
						+ SysProps.AOF_APP_BIZ_COMMENTS,
						"".getBytes(SysProps.CHARSET));
	...
	private void saveDssConfig(CcBaseEntity ccBaseEntity, String root)
		...
		ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
				+ SysProps.AOF_APP_INDEX + SysProps.AOF_APP_DS, ccBaseEntity
				.getShardIndex().toString().getBytes(SysProps.CHARSET));
		...
		for (String keyName : ccBaseEntity.getRealDataSources().keySet()) {
			...
			ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_DSS + "/"
					+ ccDataSource.getName() + SysProps.DSS_MONITOR_BASE
					+ SysProps.DSS_MONITOR_ACTIVE,
					"true".getBytes(SysProps.CHARSET));
		...
		for (Integer dsKey : ccBaseEntity.getDataSources().keySet()) {
			...
			ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
					+ SysProps.AOF_APP_DSS + "/" + dsKey + "/"
					+ SysProps.STRATEGY_NAME,
					ccDataSource.getProps().get(SysProps.STRATEGY_NAME)
							.toString().getBytes(SysProps.CHARSET));
			ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
					+ SysProps.AOF_APP_DSS + "/" + dsKey + "/"
					+ SysProps.REAl_DSS, null);
			@SuppressWarnings("unchecked")
			List<String> ll = (List<String>) ccDataSource.getProps().get(
					SysProps.REAl_DSS);
			for (String dss : ll) {
				ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
						+ SysProps.AOF_APP_DSS + "/" + dsKey + "/"
						+ SysProps.REAl_DSS + "/" + dss, null);
			}
	...
	private void saveShardingConfig(CcBaseEntity ccBaseEntity, String root)
		...
		for (String key : ccBaseEntity.getIndexTableMap().keySet()) {
			...
			for (ColumnAttribute column : l) {
				ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
						+ SysProps.AOF_APP_INDEX + SysProps.AOF_APP_TABLES
						+ "/" + key + "/" + column.getColumnName(), null);
				ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
						+ SysProps.AOF_APP_INDEX + SysProps.AOF_APP_TABLES
						+ "/" + key + "/" + column.getColumnName()
						+ SysProps.AOF_INDEX_TYPE, column.getColumnType()
						.getBytes(SysProps.CHARSET));
				ZooKeeperUtils.createSafeZKNode(root + SysProps.AOF_APP_SHARD
						+ SysProps.AOF_APP_INDEX + SysProps.AOF_APP_TABLES
						+ "/" + key + "/" + column.getColumnName()
						+ SysProps.AOF_INDEX_LENGTH,
	...
	public List<AppNode> checkAppNodesStats(ZooKeeper zk, String appName) {
		String serverPath = SysProps.ZK_MONITOR_ROOT + appName
				+ SysProps.SERVERS;
	...
	private Stat getZKSystemStat(ZooKeeper zk, String appName) throws Exception {
		ZooKeeperUtils.setZkNode(SysProps.ZK_MONITOR_ROOT + appName
				+ SysProps.TIMESTAMPS, null, zk);
		Stat stat = zk.exists(SysProps.ZK_MONITOR_ROOT + appName
				+ SysProps.TIMESTAMPS, false);
		return stat;
	}

	private AppNode getAppNode(String path, String serverPath, ZooKeeper zk,
			Stat stat) throws Exception {
		Long time = zk.exists(serverPath + "/" + path + SysProps.UPDATE, false)
				.getMtime();
		AppNode node = new AppNode();
		node.setNodeName(path);
		node.setUpdateTime(time);
		node.setSystemTime(stat.getMtime());
		node.setNodeVersion(new String(zk.getData(serverPath + "/" + path
				+ SysProps.VERSION, false, null), SysProps.CHARSET));
		node.setNodeBizVersion(new String(zk.getData(serverPath + "/" + path
				+ SysProps.BIZ_VERSION, false, null), SysProps.CHARSET));
		return node;
	}

	@Override
	public String preBaseActive(ZooKeeper zk, String appName,
			List<AppNode> nodeList) {
		try {
			String basePath = SysProps.ZK_MONITOR_ROOT + appName;
			String version = UniqueIDHolder.getIdWorker().nextId(1)
					+ SysProps.PRECOMMIT_FLAG;
			ZooKeeperUtils.startTransaction(zk);
			ZooKeeperUtils.setZkNode(basePath + SysProps.VERSION,
					version.getBytes(SysProps.CHARSET));
			for (AppNode node : nodeList) {
				ZooKeeperUtils.setZkNode(basePath + SysProps.SERVERS + "/"
						+ node.getNodeName() + SysProps.VERSION_ZK,
						version.getBytes(SysProps.CHARSET));
			}
			ZooKeeperUtils.commit();
			return version;
	...
	public List<AppNode> checkPreBaseActive(ZooKeeper zk, String appName,
		...
		try {
			...
			for (AppNode _node : nodeList) {
				AppNode node = getAppNode(_node.getNodeName(), serverPath, zk,
						stat);
				rt.add(node);
			}
			return rt;
	...
	public String commitBaseActive(ZooKeeper zk, String appName,
		...
		try {
			...
			for (AppNode node : nodeList) {
				if (version.equals("")) {
					version = node.getNodeVersion().replaceAll(
							SysProps.PRECOMMIT_FLAG, SysProps.COMMIT_FLAG);
					ZooKeeperUtils.setZkNode(basePath + SysProps.VERSION,
							version.getBytes(SysProps.CHARSET));
				}
				String nodeVersion = new String(zk.getData(basePath
						+ SysProps.SERVERS + "/" + node.getNodeName()
						+ SysProps.VERSION_ZK, false, null), SysProps.CHARSET);
				if (!node.getNodeVersion().equals(nodeVersion)) {
					throw new AofException(
							AofExceptionEnum.ZOOKEEPER_DEPLOY_ERROR);
				}
				ZooKeeperUtils.setZkNode(basePath + SysProps.SERVERS + "/"
						+ node.getNodeName() + SysProps.VERSION_ZK,
						version.getBytes(SysProps.CHARSET));
			...
			String desPath = SysProps.AOF_ROOT + "/" + appName
					+ SysProps.AOF_APP_BASE;
			String sourcePath = SysProps.AOF_TEMP_ROOT + "/" + appName + "/"
					+ tempFile;
			this.clearBaseConfig(desPath, zk, false);
			if (!ZooKeeperUtils.checkZKNodeIsExist(zk, desPath, false)) {
				ZooKeeperUtils.createSafeZKNode(desPath,
			...
			exPath.add(desPath + SysProps.AOF_APP_SHARD
					+ SysProps.AOF_APP_INDEX);
			exPath.add(desPath + SysProps.AOF_APP_SHARD
					+ SysProps.AOF_APP_INDEX + SysProps.AOF_APP_DS);
	...
	@Override
	public Map<String, String> getBizInfo(ZooKeeper zk, String appName,
			String tempPath) {
		try {
			if (tempPath == null || tempPath.equals("")) {
				return getPropertiesDataFromZooKeeper
						.getProperties(zk, appName);
			} else {
				return getPropertiesDataFromZooKeeper.getProperties(zk,
						appName, tempPath);
			}
	...
	public String preBizActive(ZooKeeper zk, String appName,
		...
		try {
			String basePath = SysProps.ZK_MONITOR_ROOT + appName;
			String version = UniqueIDHolder.getIdWorker().nextId(
					Miscellaneous.getMyid())
					+ SysProps.PRECOMMIT_FLAG;
			ZooKeeperUtils.startTransaction(zk);
			ZooKeeperUtils.setZkNode(basePath + SysProps.BIZ_VERSION,
					version.getBytes(SysProps.CHARSET));
			for (AppNode node : nodeList) {
				ZooKeeperUtils.setZkNode(basePath + SysProps.SERVERS + "/"
						+ node.getNodeName() + SysProps.BIZ_VERSION_ZK,
						version.getBytes(SysProps.CHARSET));
			}
			ZooKeeperUtils.commit();
			return version;
	...
	public String commitBizActive(ZooKeeper zk, String appName,
		...
		try {
			...
			for (AppNode node : nodeList) {
				...
				String nodeVersion = new String(zk.getData(basePath
						+ SysProps.SERVERS + "/" + node.getNodeName()
						+ SysProps.BIZ_VERSION_ZK, false, null),
						SysProps.CHARSET);
				if (!node.getNodeVersion().equals(nodeVersion)) {
					throw new AofException(
							AofExceptionEnum.ZOOKEEPER_DEPLOY_ERROR);
			...
			String desSplitPath = SysProps.AOF_ROOT + "/" + appName
					+ SysProps.AOF_APP_BIZ_SPLIT;
			String sourceSplitPath = SysProps.AOF_TEMP_ROOT + "/" + appName
					+ "/" + appName + "_" + SysProps.AOF_APP_BIZ_SPLIT_STR
					+ "/" + tempFile;
	...
	public void deleteServerNode(ZooKeeper zk, String appName, String nodeName) {
		try {
			String path = SysProps.AOF_MONITOR_ROOT + "/" + appName
					+ SysProps.SERVERS + "/" + nodeName;
			ZooKeeperUtils.startTransaction(zk);
	...
	public Boolean checkAdminUser(String userName, String pass) {
		if (userName.equals(zookeeperInit.getZooKeeperProp().getProjectName())
				&& pass.equals(zookeeperInit.getZooKeeperProp().getPassword())) {
			return true;
		}
		return false;
	}
	...
	public void importBizConfig(Map<String, CcBizEntity> ccBase, ZooKeeper zk,
		...
		try {
			...
			if (splitName == null || splitName.trim().equals("")) {
				String path = SysProps.AOF_ROOT + "/" + userName
						+ SysProps.AOF_APP_BIZ;
				Map<String, String> exist = getPropertiesDataFromZooKeeper
						.getBizSplitProperties(zk, userName);
				for (String key : ccBase.keySet()) {
					if (exist.containsKey(key)) {
						logger.error("名称为[" + key + "]已经存在!");
						throw new AofException(AofExceptionEnum.BIZ_IS_EXIST,
								new String[] { key });
					}
				}
				// 先进行清理
				...
				if (!ZooKeeperUtils.checkZKNodeIsExist(zk, SysProps.AOF_ROOT
						+ "/" + userName + SysProps.AOF_APP_BIZ_SPLIT, false)) {
					// 兼容性处理 不存在时候建立路径
					ZooKeeperUtils.createSafeZKNode(SysProps.AOF_ROOT + "/"
							+ userName + SysProps.AOF_APP_BIZ_SPLIT, null);
				}
				// 分割文件形式不清除 直接抛异常提示存在
				String path = SysProps.AOF_ROOT + "/" + userName
						+ SysProps.AOF_APP_BIZ_SPLIT + "/" + splitName;
				if (ZooKeeperUtils.checkZKNodeIsExist(zk, path, false)) {
					logger.error("BIZ分目录[" + path + "]已经存在!");
					throw new AofException(AofExceptionEnum.BIZ_SPLIT_IS_EXIST,
							new String[] { splitName });
				} else {
					Map<String, String> exist = new HashMap<String, String>();
					exist.putAll(getPropertiesDataFromZooKeeper
							.getBizSplitProperties(zk, userName));
					exist.putAll(getPropertiesDataFromZooKeeper
							.getBizProperties(zk, userName));
					for (String key : ccBase.keySet()) {
						if (exist.containsKey(key)) {
							logger.error("名称为[" + key + "]已经存在!");
							throw new AofException(
									AofExceptionEnum.BIZ_IS_EXIST,
									new String[] { key });
						}
					}
					ZooKeeperUtils.createSafeZKNode(path, null);
	...
	@Override
	public Map<String, Map<String, CcBizEntity>> getBizInfoComments(
			ZooKeeper zk, String appName, String tempPath) {
		try {
			if (tempPath == null || tempPath.equals("")) {
				return getPropertiesDataFromZooKeeper.getPropertiesWithComment(
						zk, appName);
			} else {
				return getPropertiesDataFromZooKeeper.getPropertiesWithComment(
						zk, appName, tempPath);
	...
	public void saveBizEntityToTemp(
		...
		try {
			...
			if (ccBase.containsKey(SysProps.DEDAULT_BIZ_UNIQUE_NAME)) {
				// 未分类
				this.saveBizConfigNew(
						ccBase.get(SysProps.DEDAULT_BIZ_UNIQUE_NAME), path);
				ccBase.remove(SysProps.DEDAULT_BIZ_UNIQUE_NAME);
			}
			// 其他情况-split
			path = SysProps.AOF_TEMP_ROOT + "/" + appName + "/" + appName + "_"
					+ SysProps.AOF_APP_BIZ_SPLIT_STR;
			if (!ZooKeeperUtils.checkZKNodeIsExist(zk, path, false)) {
				ZooKeeperUtils.createSafeZKNode(path, null);
			}
			path = path + "/" + fileName;
	...
	public void saveBizEntity(Map<String, Map<String, CcBizEntity>> ccBase,
		...
		try {
			...
			path = SysProps.AOF_ROOT + "/" + appName
					+ SysProps.AOF_APP_BIZ_SPLIT;
			if (!ZooKeeperUtils.checkZKNodeIsExist(zk, path, false)) {
				ZooKeeperUtils.createSafeZKNode(path, null);
			}
	...
	public void notifyDssMonitor(ZooKeeper zk, String appName, String password) {
		...
		// ZooKeeperUtils.startTransaction(zk);
		// ZooKeeperUtils.setACL(this.generateACL(appName, password));
		// ZooKeeperUtils.setZkNode(path+SysProps.AOF_APP_DSS,
		// SysProps.DSS_CHANGE_BY_INFO.getBytes(SysProps.CHARSET));
		// ZooKeeperUtils.commit();
		// } catch (Exception e) {
		// logger.error(e.getMessage(), e);
		// throw new AofException(AofExceptionEnum.ZOOKEEPER_USE_ERROR);
		 ...
		 */
		String path=SysProps.AOF_DB_MONITOR+"/"+appName;
		if(ZooKeeperUtils.checkZKNodeIsExist(zk, path, false)){
			ZooKeeperUtils.setZkNode(path, null, zk);
		}
	}
