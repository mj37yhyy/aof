...
public class AofSessionListener implements HttpSessionListener {
	...
	public void sessionDestroyed(HttpSessionEvent se) {
		...
		if(se.getSession().getAttribute(AofCcProps.SESSION_FLAG_RUN)!=null){
			isRunMode=(Boolean)se.getSession().getAttribute(AofCcProps.SESSION_FLAG_RUN);
		}
		if(isRunMode){
			if(se.getSession().getAttribute(AofCcProps.SESSION_APP)!=null){
				...
				logger.info("应用["+app+"]运行模式调用退出或者session超时");
			...
			if(se.getSession().getAttribute(AofCcProps.SESSION_APP)!=null){
				//开发模式没有缓存ZK连接 无需清理 直接打日志退出
				String app=(String)se.getSession().getAttribute(AofCcProps.SESSION_APP);
				logger.info("应用["+app+"]开发模式调用退出或者session超时");
			}
		}
